...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("ThotMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("ThotMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("ThotMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("thot-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running ThotMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
